...
const eco = new Economy({
  ...
  module.exports={
    ...
    const amount_embed = new EmbedBuilder() 
    .setColor("#FF0000")
    .setTitle("You have to specify a an amount, noob")
    .setTimestamp()
    if (!amount) return message.channel.send({embeds:[amount_embed]})
    const must_be_number_embed = new EmbedBuilder()
    .setColor("#FF0000")
    .setTitle("The amount must be a number, don't try to break me")
    .setTimestamp()
    if (isNaN(amount)) return message.channel.send({embeds:[must_be_number_embed]})
    const not_enough_embed = new EmbedBuilder()
    .setColor("#FF0000")
    .setTitle(`You don't even have enough money in your bank to withdraw **${amount}** smh`)
    .setTimestamp()
    if (amount > balance) return message.channel.send({embeds:[not_enough_embed]})
        
    eco.balance.add(amount, message.author.id, message.guild.id)
    eco.bank.subtract(amount, message.author.id, message.guild.id)
    const sent_money_embed = new EmbedBuilder()
    .setColor("#FF0000")
    .setTitle(`Successfully sent **${amount}** pepsi's to your balance`)
    .setTimestamp()
    message.channel.send({embeds:[sent_money_embed]})
