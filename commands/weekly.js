...
const eco = new Economy({
  ...
  module.exports={
    ...
    execute(message, args){
        ...
        .setColor("#FF0000")
        .setDescription(`You have already claimed your daily reward! Time left until next claim: **${weekly.value.days}** days, **${weekly.value.hours}** hours, **${weekly.value.minutes}** minutes and **${weekly.value.seconds}** seconds.`)
        .setTimestamp()
        if (!weekly.status) return message.channel.send({embeds:[claimed_weekly_embed]})
        const weekly_embed = new EmbedBuilder()
        .setColor("#FF0000")
        .setDescription(`You have received **${weekly.reward}** daily coins!`)
        .setTimestamp()
